...
pass log (all to pflog0) on { vtnet1 vtnet2 }"
	...
	tap_auto client tapA "${aprefix}.1/28" \
			vtnet0 "${aprefix}.2/28"
	tap_auto server tapB "${aprefix}.17/28" \
			vtnet0 "${aprefix}.18/28"
	tap_auto client tapC "${aprefix}.33/28" \
			vtnet1 "${aprefix}.34/28"
	tap_auto server tapD "${aprefix}.35/28" \
			vtnet1 "${aprefix}.36/28"
	tap_auto client tapE "${aprefix}.49/28" \
			vtnet2 "${aprefix}.50/28"
	tap_auto server tapF "${aprefix}.51/28" \
			vtnet2 "${aprefix}.52/28"
	tap_auto client tapG "${aprefix}.65/28" \
			vtnet3 "${aprefix}.66/28"
	tap_auto server tapH "${aprefix}.67/28" \
